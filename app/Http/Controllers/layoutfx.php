...
if($request['cat_Name']){
     ...
     }elseif($request['Destinations']){


$social=social::get();

$card=box::paginate(20);
...
$Allhaveparent=nav::where('parent_id','<>',0)->get();
...
searchbar::create([
      ...
      if( $countnav > 0){
        ...
        return view('/index'
        ,compact('nav','nav_sub','nav_subm','social','Maxcompanyid','Keywords','footernav','Allhaveparent','navNullfooter','card','box','boxes'),[
       
       
        'cat_type' =>'searchResult',
        'page_title' =>'Discover Travel Packages',
        'BSModel'=> $BusinessModel,
        'Domain_site'=> $Domain_site,
        'container'=>$container,
        'bgimage'=>$bgimage,
    ...
    foreach($boxes as  $boxesitem ){
      ...
      $KWsearch=$request->search_bt;

  
      $card=box::leftJoin
      ('keywords' , 'boxes.ID' , '=', 'keywords.box_id')->where('Keyword','like','%'.$KWsearch.'%')->paginate(20);
...
$box=box::leftJoin
     ...
     // ('keywords' , 'navs.id' , '=', 'keywords.nav_id')->where('Keyword','like','%'.$KWsearch.'%')->get();
      ...
      //('keywords' , 'topics.id' , '=', 'keywords.topic_id')->where('Keyword','like','%'.$KWsearch.'%')->get();
      ...
      if( $countnav > 0){
        ...
        return view('/index'
        ,compact('nav','nav_sub','nav_subm','social','Maxcompanyid','Allhaveparent','Keywords','footernav','navNullfooter','box','card','boxes'),[
 ...
 ,compact('nav','nav_sub','nav_subm','cover','Keywords','social','Maxcompanyid','navNullfooter','footernav','marque','paragraph','topic','box','boxes','sliderMin','sliderMax','slider','static','commonposts','Allboxes','FAQS'),[
  ...
  if(!empty($parent_id)){
      ...
      $card= box::where ('nav_id',$page_id)->paginate(20);
 ...
 return view('/index'
 ,compact('nav','nav_sub','nav_subm','cover','social','Keywords','Maxcompanyid','navNullfooter','footernav','marque','paragraph','topic','card','box','boxes','sliderMin','sliderMax','slider','static','FormOption','commonposts','AllArticle','Allhaveparent','FAQS'),[
  ...
  'navID' => $page_id ,
   'boxesitem'=>'true'
    ...
    $DayTime= $breadcrumbitem->DayTime  ;
       ...
       $Duration= $breadcrumbitem->Duration;

    $class= $breadcrumbitem->class  ;

     $Country=$breadcrumbitem->Country  ;

    $Run= $breadcrumbitem->Run  ;

    $Desc= $breadcrumbitem->Desc;
   ...
   // $company=company::where ('nav_id',1)->get();
    ...
    $topic=topic::where ('box_id',$box_id)->get();
     ...
     $Maxcompany_id=company::Max('id')->get();
      ...
      $sliderMax=slider::where('box_id',$box_id)->whereRaw("id > (select Min(id)from sliders where box_id=$box_id )")->get();
 ...
 return view('/index'
 ,compact('nav','nav_sub','nav_subm','cover','static','social','Keywords','Maxcompanyid','navNullfooter','footernav','paragraph','topic','box','boxes','sliderMin','sliderMax','slider','FAQS'),[
