...
<select  class="btn btn-secondary dropdown-toggle" style =" margin-right:10px; display: flex; align-items: center; justify-content: center;background:#bbbbbb; color:white;" id="langOpt" style="margin-right:10px;">
...
style ="display: flex; align-items: center; justify-content: center;background:#bbbbbb; color:white;" name='LinkPosition'  require>
...
<td>
    ...
    <input type="SUBMIT" name="UPDATE"  VALUE="UPDATE"   class="btn " style='background:#bbbbbb; color:white;'  >
    </form>
    </div>
    </td>
...
@endif
<input type="SUBMIT" name="SELECT" value="Add" class="btn " style='background:#bbbbbb; color:white;' />
</form>
@endif
</td>
<td>
@if(empty($navitem->footer))
<a  style ="display: flex; align-items: center; justify-content: center;"  href="http://{{$Domain_site}}/index.php?MainM_ID=<?php echo $navitem->id;?>" target='_blank'>
<input type="submit" title="VIEWPAGE" value="View Page" class="btn " style='background:#bbbbbb; color:white;' >
</a>
@endif
</td>
@endif
...
<td>
@if(empty($navitem->footer))
<a  style ="display: flex; align-items: center; justify-content: center;"  href="https://freetools.seobility.net/en/seocheck/{{$Domain_site}}" target='_blank'>
<input type="submit" name="selecttoupdate" class="update btn" title="VIEW SEO REport" value="View SEO Report" style='background:#bbbbbb; color:white;'>
...
<form action="{{route('nav.destroy',$navitem->id)}}" method="Post">
@csrf
@method('DELETE')
@if(!empty($navID))
<input    name='parent_id'  type='hidden' value="{{$navID}}" />
...
<input type="SUBMIT" name="INSERT" VALUE="Edit"  class="btn btn-primary" style='background:blue' ></div>
</form>  @ENDIF
</td>
